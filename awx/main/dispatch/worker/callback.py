...
from awx.main.tasks.system import ensure_success_and_failure_notifications
                ...
                if body.get('event') == 'EOF':
                    try:
                        ...
                        if isinstance(uj, Job):
                            ...
                            ensure_success_and_failure_notifications(uj.id)
                    except Exception:
                        logger.exception('Callback worker failed to emit notifications: Job {}'.format(job_identifier))
                    finally:
                        self.subsystem_metrics.inc('callback_receiver_events_in_memory', -1)
                        set_guid('')
                    return
