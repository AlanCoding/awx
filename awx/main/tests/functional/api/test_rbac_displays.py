...
def test_prefetch_jt_capabilities(job_template, rando):
    ...
    mapping = prefetch_page_capabilities(JobTemplate, qs, [{'edit': 'change'}, {'start': 'execute_jobtemplate'}], rando)
...
def test_prefetch_ujt_job_template_capabilities(alice, bob, job_template):
    ...
    mapping = prefetch_page_capabilities(UnifiedJobTemplate, qs, [{'edit': 'change'}, {'start': 'execute_jobtemplate'}], alice)
    assert mapping[job_template.id] == {'edit': False, 'start': True}
    qs = UnifiedJobTemplate.objects.all()
    mapping = prefetch_page_capabilities(UnifiedJobTemplate, qs, [{'edit': 'change'}, {'start': 'execute_jobtemplate'}], bob)
...
def test_prefetch_group_capabilities(group, rando):
    ...
    mapping = prefetch_page_capabilities(Group, qs, [{'edit': 'inventory.change'}, {'adhoc': 'inventory.adhoc_inventory'}], rando)
